...
import { 
  Cpu, GraduationCap, School, ShieldAlert,
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        toast({ title: "Account Created", description: `Welcome to Codeathon AI, ${name}!` });
      } else {
        await signInWithEmailAndPassword(auth, email, password);
        toast({ title: "Welcome Back", description: "Connection verified." });
      }
      router.push('/dashboard');
    } catch (error: any) {
      setIsAuthenticating(false);
      ...
      if (error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') errorMessage = "Invalid email or password.";
      
      toast({ variant: "destructive", title: "Auth Error", description: errorMessage });
    }
  };
        ...
        <Card className="border-white/10 bg-black/40 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden shadow-2xl border">
          <CardContent className="p-8 md:p-14 space-y-8">
                ...
                {isSignUp && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-7 overflow-hidden">
                    <div className="space-y-2">
                      <Label className="text-[10px] uppercase font-bold text-blue-400/60 tracking-[0.2em]">Full Name</Label>
                      <div className="relative">
                        <UserIcon className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-white/20" />
                        <Input placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} className="pl-14 bg-white/[0.03] border-white/10 rounded-2xl h-14" />
                      </div>
                    </div>

                    <div className="space-y-4">
                      <Label className="text-[10px] uppercase font-bold text-blue-400/60 tracking-[0.2em]">User Role</Label>
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input type="email" placeholder="name@domain.com" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-14 bg-white/[0.03] border-white/10 rounded-2xl h-14" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] uppercase font-bold text-blue-400/60 tracking-[0.2em]">Password</Label>
                  <div className="relative">
                    <Lock className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-white/20" />
                    <Input type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-14 bg-white/[0.03] border-white/10 rounded-2xl h-14" />
                  </div>
                </div>
              </div>

              <Button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-purple-700 hover:from-blue-500 hover:to-purple-600 border-0 rounded-2xl h-16 font-bold text-white group" disabled={isAuthenticating}>
                {isAuthenticating ? <Loader2 className="h-5 w-5 animate-spin" /> : <span className="tracking-widest uppercase text-xs">{isSignUp ? 'Register Account' : 'Sign In'}</span>}
              </Button>
            </form>

            <div className="relative py-4">
              <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/5"></span></div>
              <div className="relative flex justify-center text-[9px] uppercase font-bold tracking-[0.3em]"><span className="bg-[#020617] px-4 text-white/20">OR</span></div>
            </div>

            <Button variant="outline" className="w-full border-white/10 rounded-2xl h-14 bg-white/[0.02] hover:bg-white/5 text-white/60 flex items-center justify-center gap-3" onClick={loginWithGoogle} disabled={isAuthenticating}>
            ...
            <div className="text-center">
              <button onClick={() => setIsSignUp(!isSignUp)} className="text-[10px] uppercase font-bold tracking-[0.2em] text-white/30 hover:text-blue-400">
